...
internals.Object = class extends Any {
    ...
    _base(value, state, options, next) {
        ...
        Async.serie([
            ...
            (cb) => {
                ...
                unprocessed = new Set(Object.keys(target));

                if (this._inner.children) {
                    const stripProps = [];

                    Async.for(this._inner.children, (child, i, nextChild) => {
                        ...
                        child.schema._validate(item, localState, options, null, (result) => {

                            if (result.errors) {
                                errors.push(this.createError('object.child', { key, child: child.schema._getLabel(key), reason: result.errors }, localState, options));

                                if (options.abortEarly) {
                                    return nextChild(true);
                                }
                            }
                            else {
            ...
            (cb) => {
                ...
                if (unprocessed.size && this._inner.patterns.length) {
                    ...
                    Async.for(unprocessed, (key, i, nextKey) => {
                        ...
                        Async.for(this._inner.patterns, (pattern, j, nextPattern) => {

                            if (pattern.regex.test(key)) {
                                unprocessed.delete(key);

                                pattern.rule._validate(item, localState, options, null, (result) => {
    ...
    assert(ref, schema, message) {
        ...
        return this._test('assert', { schema, ref }, function (value, state, options, next) {

            schema._validate(ref(value), null, options, value, (result) => {
                ...
                return next(this.createError('object.assert', { ref: path, message }, localState, options));
            });
        });
    }
